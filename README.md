...
**功能3**  通过 拼音状态下```uU```字母来引导拆字模式 举例 ```震```  假设你不认识，你可以通过雨和辰 来合并输入

![截图_选择区域_20240704150839.png](https://storage.deepin.org/thread/202407041150405343_截图_选择区域_20240704150839.png)

拼音状态输入后，继续输入u会消失如下图，输入 ```yu if``` 即雨 辰，结果出现了我们要的震字，且给出了辅助码 ```y``` 和  ```i```  ，y是雨的声母y，i是辰的声母ch
